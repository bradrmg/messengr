...
switch (_OP_) {		
	case "user_inbox":
		...
		if ($in_sender = trim($_REQUEST['in_sender'])) {
			...
			//$list = dba_search(_DB_PREF_ . '_tblSMSInbox', 'in_id, in_uid, in_datetime, in_sender, in_msg', $conditions, $keywords, $extras);
			//$list  = conversation_view('Test this String');
			//$list = 'Hello';
			//$list = conversation_view("1");
			//echo $list;
			//var_dump($list);
		} else {
			$search = themes_search($search_category, $base_url);
		...
		for ($j = 0; $j < count($list); $j++) {
			...
			$convo_link_text = "convo test";
			$convo_link = "";
			$convo_link = $convo_link = "<a href='" . $base_url . "&in_sender=" . urlencode($in_sender) . "'>" . sprintf(_('view all %d'), $message_count) .$convo_link_text. "</a>";
			$view_all_link = "";
			if ($message_count > 1) {
